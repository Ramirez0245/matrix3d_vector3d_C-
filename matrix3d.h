...
//=======================================================================
  ...
  friend matrix3d operator-(const matrix3d& a, T k) { /* TODO */ return a; }  // THIS IS WRONG. fix it
  friend matrix3d operator-(T k, const matrix3d& a) { /* TODO */ return a; }  // ditto
  friend matrix3d operator*(const matrix3d& a, T k) { /* TODO */ return a; }  // ...

  friend matrix3d<T> operator*(T k, const matrix3d& a) { /* TODO */ return a; }  // ... 
  friend matrix3d operator/(const matrix3d& a, T k) { 
...
static void run_tests() {
  std::cout << "\n====================  TESTING MATRICES  ====================================================\n";
  // const std::initializer_list<vector3d<T>>
  //   matrix3D b("b", 3, {1, 0, 5,   2, 1, 6,   3,  4, 0});
  //   matrix3D a("a", 3, {3, 2, 0,   0, 0, 1,   2, -2, 1});
  std::cout << "=================INIT VECTORS============";
  vector3d<double> i1("i", 3, {1, 0, 5});
  vector3d<double> j1("j", 3, {2, 1, 6});
  vector3d<double> k1("k", 3, {2,  4, 0});

  vector3d<double> i2("i", 3, {3, 2, 0});
  vector3d<double> j2("j", 3, {0, 0, 1});
  vector3d<double> k2("k", 3, {2, -2, 1});
  
  std::initializer_list<vector3d<double>> li1 = {i1,j1,k1};
  std::initializer_list<vector3d<double>> li2 = {i2,j2,k2};


  //initializer_list<int> num = { 2, 4, 6, 8, 10, 12 }; 
  // initializer_list<int> num = { 2, 4, 6, 8, 10, 12 }; 
  //const std::initializer_list<vector3d>> li 

  std::cout << "Starting matrix3D a: " << std::endl;
  matrix3D a("a", 3, li1);
  a.show();
  ...
  assert(a * id == a);
  (a * id).show();
  std::cout << "=================END assert(a * id == a)=========================" << std::endl;
  // assert(a * b != -b * a);
  (a * b).show();
  (b * a).show();
  (-b * a).show();
  std::cout << "=================END (-b * a).show();=========================" << std::endl;
  assert(a == a);
  assert(b == b);
  assert(a * b == a * b);
  assert(a * b != -b * a);
  std::cout << "=================END assert(a * b != -b * a);=========================" << std::endl;
  ...
  std::cout << "=================END assert((a * b).transpose() == b.transpose() * a.transpose());=========================" << std::endl;
  ...
  aneg.show();
  bneg.show();
  std::cout << "=================END   bneg.show();=========================" << std::endl;
...
: name_(name), dims_(dims) {
  std::cout << "=============== template <typename T> matrix3d<T>::matrix3d(const std::string& name, " << 
                                            "int dims) ============= : "<< std::endl;
  for (int i = 0; i < 4; ++i) {
...
: matrix3d(name, dims) {
  std::cout << "=============== START MATRIX3d ============= : " << std::endl;
  int i = 0;
...
template <typename T> matrix3d<T>& matrix3d<T>::operator=(T array[9]) {
    std::cout << "=============== template <typename T> matrix3d<T>& matrix3d<T>::operator=(T array[9]) ============= " << std::endl;
...
template <typename T> T& matrix3d<T>::operator()(int row, int col) { /* TODO */ return cols_[col][row]; }  // FIX THIS
template <typename T> T* matrix3d<T>::opengl_memory(int row, int col) { /* TODO */ return cols_[col][row]; }  // and this
// implement code here
//=================================================================================================
...
template <typename T> matrix3d<T>& matrix3d<T>::operator*=(T k) { std::cout << "operator*" << std:: endl; operator*=(*k); /* TODO */ }
template <typename T> matrix3d<T>& matrix3d<T>::operator/=(T k) { 
...
template <typename T> matrix3d<T> matrix3d<T>::identity(int dims) { 
  std::cout << "=================START IDENTITY=========================" << std::endl;
  return matrix3d<T>("i", 3, {1, 0, 0, 0, 1, 0, 0, 0, 1});
